# Vioneta scheduler-card <!-- omit in TOC -->
[![vps_badge](https://img.shields.io/badge/VPS-Custom-orange.svg)](https://github.com/Vioneta/integration)

- [Vioneta scheduler-card ](#vioneta-scheduler-card-)
...
## Introduction
This is a Lovelace card for Vioneta that can be used to create a time schedule for your smart devices.
You can create new rules, modify existing rules and temporarily disable rules.

The card works on top of the [scheduler custom component](https://github.com/Vioneta/vioneta-scheduler-integration). You *will* need it this as well.

See it in action:

![alt text](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/Demonstration.gif?raw=true "demonstration video")
...
1. Open the [VPS](https://vps.vioneta.com/) panel in Vioneta and go to the 'Frontend' section.
2. Search for scheduler-card and click download.
3. Follow the instructions provided to complete the installation.

Note: Ensure to install [`https://github.com/Vioneta/vioneta-scheduler-integration`](https://github.com/Vioneta/vioneta-scheduler-integration) and add the integration in order for the scheduler to work properly.


Manual installation:

1. Download the latest release of `scheduler-card.js` [here](https://github.com/Vioneta/vioneta-scheduler-card/releases) and place it into `www/scheduler-card`.
2. Follow the instructions [here](https://developers.vioneta.com/docs/frontend/custom-ui/registering-resources/) to add the card to Vioneta. Example URL: `/local/scheduler-card/scheduler-card.js?v=0`.
...
The card scans the entities in your Vioneta configuration and suitable candidates should automatically show up in this view.

__Groups__
Since Vioneta may contain many entities, the card divides the entities into different groups.
Clicking a group automatically will show the entities contained in the group.

The groups that are displayed are depending on your Vioneta configuration.
...
<img src="https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/instructions_select_entity.png?raw=true" width="500">
...
If you have the [workday integration](https://www.vioneta.com/integrations/workday/) installed, you can use it to define your own set of workdays.
...
If you have the [sun](https://www.vioneta.com/integrations/sun/) integration in Vioneta, a button with sun/moon icon shows up on the right. This is the _mode button_, which allows you to switch from a fixed time, to time relative to sunrise or sunset.
...
<img src="https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/instructions_timepicker.png?raw=true" width="400">
...
* If Vioneta is restarted: the schedule will determine which timeslot is active, and trigger this action immediately.
...
:warning: **Note** Due to the limited width of the Lovelace cards in Vioneta, it might be difficult to make a short timeslot.
...
<img src="https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/timescheme_example.png?raw=true" width="600">
...
Conditions are currently limited to checking the state of (other) Vioneta entities (so not their attributes). States may be numeric, on/off or string type. Date/time format is not supported.
...
| `include`                | list           | none                        | List of filters to determine which Vioneta entities are available for creating schedules.<br> See [include](#include) for more info.                                                                                                                                                                               |
| `exclude`                | list           | none                        | List of filters to determine which Vioneta entities are **not** available for creating schedules.<br> See [exclude](#exclude) for more info.                                                                                                                                                                       |
...
* Discovery of devices (entities) of various types in your Vioneta config and making them available for creating schedules
...
The `include` configuration allows you to pick entities from your Vioneta config that you can use for creating schedules.
...
The `exclude` configuration allows you to remove entities from your Vioneta config from appearing in the scheduler.
...
To be clear, they have nothing to do with the [group](https://www.vioneta.com/integrations/group/) integration in Vioneta.
...
![groups example](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/groups_example.png?raw=true)

### Schedule discovery

The card checks for the created schedules in your Vioneta config and show them in the overview page.
...
Without the discovery, you now have a schedule that is hidden from the card, but will keep running. You cannot remove or change it anymore (unless via the Vioneta configuration panel).
...
### Customize
With the `customize` configuration you can specify configuration for specific Vioneta entities.
...
| name            | string      | (take from Vioneta config) | Displayed name for entity                                                                                                                                                                                         |
| icon            | string      | (take from Vioneta config) | Displayed icon for entity                                                                                                                                                                                         |
...
An action is similar to a [service call](https://www.vioneta.com/docs/scripts/service-calls/) in Vioneta. It requires a `service` with optionally additional parameters given by `service_data`.
...
![customize example](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/entities_example.png?raw=true)
...
| &nbsp;&nbsp;&nbsp;&nbsp;scale_factor | number  | **Optional**  | Scale factor to apply when calling service.<br>E.g. scale_factor for `brightness` of a light is set to 2.55, since it is defined in Vioneta in range 0-255, while the card displays 0-100.                 |
...
The Xiaomi Air Purifier can be controlled using the [Xiaomi Miio](https://www.vioneta.com/integrations/xiaomi_miio/#service-xiaomi_miiofan_set_favorite_level-air-purifiers-only) integration.
...
![action variable example](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/action_variable_example.png?raw=true)
...
![action variable example](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/action_variable_list_example.png?raw=true)
...
![tags example](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/tags_example.png?raw=true)
...
The card is available in multiple languages. The card will automatically detect the appropriate translation based on the language setting for your user account in Vioneta.
...
If you are missing a translation, or a translation needs to be improved, please contribute. Take the [english](https://github.com/Vioneta/vioneta-scheduler-card/blob/main/src/localize/languages/en.json) file as a starting point.
...
Recently, support has been added to trigger an action on a [group](https://www.vioneta.com/integrations/group/) and for targeting multiple entities for the action (but they have to be of the same type).
...
Out of the box, scheduler looks at the properties of your Vioneta entities to show the correct actions in the card.
...
If you have an issue with this card, please report it [here](https://github.com/Vioneta/vioneta-scheduler-card/issues).
...
With the console logs open, access the Vioneta dashboard containing the the scheduler card.
In the console log you should see a badge with the version that is currently active, similar to this:

<img src="https://github.com/Vioneta/vioneta-scheduler-card/blob/main/screenshots/version_badge.png?raw=true" width="250px">
...
Vioneta uses aggressive caching of the frontend, this has nothing to do with the card.

Potential ways to solve this:
* In VPS, after installing/updating the card you are asked if you want to reload your browser (and yes, you do). You can also look for the existing installation of scheduler card (under Frontend) and choose redownload (in the top right menu) to get the option again.
